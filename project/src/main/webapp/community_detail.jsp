...
<body>
	...
	<section id="portfolio" class="darker">
		<div class="container2">
			...
			<!-- 커뮤니티 상세내용 -->
				<br>
				<h5>커뮤니티 <a href="community.jsp">뒤로가기</a></h5> 
				<h5 id="board_num"><%=dto.getBoard_num() %></h5> 
				<h5>제목 : <%=dto.getBoard_title() %></h5> 
				<h5>카테고리 :<%=dto.getBoard_category() %></h5> 
				<h5>작성자email :<%=dto.getMb_email() %></h5> 
				<h5>게시날짜 :<%=dto.getBoard_date() %></h5> 
			...
			<div class="row">
				<!--  <figure class="portfolio-item col-md-4 col-sm-6"> -->
					<img class="img-responsive" style="height: 500px; width: 500px; margin-left: 30%;" src="./file/<%=dto.getBoard_file()%>" alt="Adena icons pack" />
				...
				<%if(info!=null){ %>
				<div>
				<form action="CommentService?board_num=<%=dto.getBoard_num() %>" method="post">
				<div>작성자이메일 :<%=info.getMb_email() %></div>
				<textarea type="text" name="comment_content"  id="content" style="resize: none;" rows="10" ></textarea>
				...
				<%for(int i =0; i<list.size(); i++){ %>
					<tr> <th><%=i+1 %></th> </tr>
					<tr> 
					<td> <span>작성자이메일 <%=list.get(i).getMb_email()%> </span> </td>
